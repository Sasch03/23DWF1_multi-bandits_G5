...
describe("EpsGreedy eyeball logs", () => {
    it("Bernoulli (rounded logs)", () => {
        ...
        game.setNumberOfArms(k); game.setNumberOfTries(T);
        game.setChosenDistribution(DistributionTyp.BERNOULLI);
        game.setChosenAlgorithms([]); game.createTable();

        epsilonProvider.setEpsilon(0.2);
        const algo = new EpsGreedy({ numberOfArms:k, numberOfTries:T });

        console.log("=== Bernoulli table ==="); logTable(game.tableOfRewards);
        const out = runOnTable(game.tableOfRewards, algo);
        let s=0; const cum = out.rewards.map(r => (s+=r, s));
    ...
    it("Gaussian (rounded logs)", () => {
        ...
        console.log("=== Gaussian table (rounded) ==="); logTable(game.tableOfRewards, true);
        const out = runOnTable(game.tableOfRewards, algo);
        let s=0; const cum = out.rewards.map(r => (s+=r, s)).map(r2);
